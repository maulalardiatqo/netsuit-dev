 ...
 * **Copyright (c) 2025 ABJ Cloud Solutions, Inc.
 * @Client        :  Suy Sing
 * @Script Name   :  - SSCC | RL | Get PROMOS
 * @script File   :  abj_rl_get_promos.js
 * @Trigger Type  :  Get Lead Source
    ...
    */
    function get(requestParams) {
        try{
            ...
            var promosSearch = search.load({
                id: 'customsearch1816'
            });

            var pagedData = promosSearch.runPaged({
                pageSize: pageSize
            });
            ...
            page.data.forEach(function (result) {
                var idPromo = result.getValue({
                    name: 'id'
                });
                var enrollStartDate = result.getValue({
                    name: 'custrecord_promo_enroll_start_date'
                });
                var enrollEndDate = result.getValue({
                    name: 'custrecord_promo_enroll_dc'
                });
                var vendor = result.getValue({
                    name: 'custrecord_promo_enroll_vendor'
                });
                var promoDesc = result.getValue({
                    name: 'custrecord_promo_enroll_description'
                });
