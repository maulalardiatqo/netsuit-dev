 ...
 * **Copyright (c) 2025 ABJ Cloud Solutions, Inc.
 * @Client        :  Suy Sing
 * @Script Name   :  - SSCC | RL | Get Items
 * @script File   :  abj_rl_get_items.js
    ...
    function get(requestParams) {
        try{
            ...
            var itemsSearch = search.load({
                id: 'customsearchwebsite_items_get'
            });
            var pagedData = itemsSearch.runPaged({
                pageSize: pageSize
            });
            ...
            page.data.forEach(function (result) {
                ...
                var itemDescription = result.getValue({
                    name: "custitem_abj_full_description"
                });
                var itemMainBrand = result.getValue({
                    name: "custitem_abj_mainbrand"
                });
                var mainCategoryCode = result.getValue({
                    name: "custitem_abj_maincat"
                });
                var vendor = result.getValue({
                    name: "othervendor"
                });
                var unitsType = result.getValue({
                    name: "unitstype"
                });
