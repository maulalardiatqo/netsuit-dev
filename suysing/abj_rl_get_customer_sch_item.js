 ...
 * **Copyright (c) 2025 ABJ Cloud Solutions, Inc.
 * @Client        :  Suy Sing
 * @Script Name   :  - SSCC | RL | Get CUSTOMER SCHEDULE ITEMS
 * @script File   :  abj_rl_get_customer_sch_item.js
    ...
    */
    function get(requestParams) {
        try{
            ...
            var custSchSearch = search.load({
                id: 'customsearchwebsite_customer_sch_get'
            });

            var pagedData = custSchSearch.runPaged({
                pageSize: pageSize
            });
            ...
            page.data.forEach(function (result) {
                ...
                var transactionType = result.getValue({
                    name: "custbody_abj_cm_transtype"
                });
                var type = result.getValue({
                    name: 'custbody_abj_ttype'
                });
                var location = result.getValue({
                    name: 'location'
                });
                var scheduleDate = result.getValue({
                    name: 'custcol_abj_sch_date'
                });
                results.push({
