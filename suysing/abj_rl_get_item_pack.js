 ...
 * **Copyright (c) 2025 ABJ Cloud Solutions, Inc.
 * @Client        :  Suy Sing
 * @Script Name   :  - SSCC | RL | Get Item Packages
 * @script File   :  abj_rl_get_item_packages.js
    ...
    */
    function get(requestParams) {
        try{
            ...
            var itemPackSearch = search.load({
                id: 'customsearchwebsite_item_pack_get'
            });
            var pagedData = itemPackSearch.runPaged({
                pageSize: pageSize
            });
            ...
            page.data.forEach(function (result) {
                var names = result.getValue({
                    name: "name"
                });
                var unitName = result.getValue({
                    name: "unitname"
                });
                var unitNamePlural = result.getValue({
                    name: "pluralname"
                });
                var abbreviationName = result.getValue({
                    name: "abbreviation"
                });
                var abbreviationNamePlural = result.getValue({
                    name: "pluralabbreviation"
                });
                var rate = result.getValue({
                    name: "conversionrate"
                });
                var isBaseUnit = result.getValue({
                    name: "baseunit"
                });
                ...
                results.push({
                    ...
                    abbreviationNamePlural : abbreviationNamePlural || '',
                    rate : rate || '',
