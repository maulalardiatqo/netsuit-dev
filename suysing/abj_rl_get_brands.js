 ...
 * **Copyright (c) 2025 ABJ Cloud Solutions, Inc.
 * @Client        :  Suy Sing
 * @Script Name   :  - SSCC | RL | Get Brands
 * @script File   :  abj_rl_get_brands.js
    ...
    */
    function get(requestParams) {
        try{
            ...
            var brandSearch = search.load({
                id: 'customsearchwebsite_account_brands_get'
            });

            var pagedData = brandSearch.runPaged({
                pageSize: pageSize
            });
            ...
            page.data.forEach(function (result) {
                var itemMainBrandId = result.getValue({
                    name: 'custitem_abj_mainbrand'
                });
                var itemMainBrandText = result.getText({
                    name: 'custitem_abj_mainbrand'
                });

                var brandDescription = result.getValue({
                    name: 'custitem_abj_brand_description'
                });

                results.push({
                    itemMainBrandId: itemMainBrandId || '',
                    itemMainBrandText : itemMainBrandText || '',
