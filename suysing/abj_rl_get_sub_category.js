 ...
 * **Copyright (c) 2025 ABJ Cloud Solutions, Inc.
 * @Client        :  Suy Sing
 * @Script Name   :  - SSCC | RL | Get SUB Category
 * @script File   :  abj_rl_get_sub_category.js
    ...
    */
    function get(requestParams) {
        try{
            ...
            var subCategorySearch = search.load({
                id: 'customsearchwebsite_sub_category_get'
            });

            var pagedData = subCategorySearch.runPaged({
                pageSize: pageSize
            });
            ...
            page.data.forEach(function (result) {
                var mainCategory = result.getValue({
                    name: 'custitem_abj_maincat'
                });
                var subCategoryCode = result.getValue({
                    name: 'custitem_abj_subcat'
                });
                var subCategoryDesc = result.getValue({
                    name: 'custitem_abj_subcat_desc'
                });
                var remarks = result.getValue({
                    name: 'custitem_abj_remarks'
                });
                var itemTagging = result.getValue({
                    name: 'custitem_abj_item_tagging'
                });
