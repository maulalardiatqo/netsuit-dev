    ...
    function onRequest(context) {
      if (context.request.method === "GET") {
        ...
        try {
          ...
          if (fileData) {
            fileData.folder = 5531;
            var file_id = fileData.save();
            log.debug({
              title: "file_id",
              details: file_id,
            });
            if (file_id) {
              ...
              var fileLineCount = lines.length;
              log.debug("File Line Count", fileLineCount);
              lines.forEach(function (line, index) {
                log.debug("index", index);
                if (index > 0) {
                  log.debug("valuees", line);
              ...
              if (success == true) {
                ...
                var form_result = serverWidget.createForm({
                  title: "Result Upload Item Prices",
                });
