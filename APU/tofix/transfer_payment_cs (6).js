...
define([
  ...
  function loadData() {
    ...
    if (vrecord.getValue("custpage_customer") == '') {
      ...
      var filterItem = vrecord.getValue("custpage_item_no");
      var filterItemIn = vrecord.getValue("custpage_item_no_in");
      var filterItemRecord = vrecord.getValue("custpage_item_record");
  ...
  function fieldChanged(context) {
    ...
    if(context.fieldId == "custpage_customer" || context.fieldId =="custpage_transdatefrom" || context.fieldId == "custpage_transdateto" || context.fieldId == "custpage_item_no" || context.fieldId == "custpage_item_no_in" || context.fieldId == "custpage_item_record"){
      loadData();
    }
    if (context.sublistId == "custpage_sublist_out" && context.fieldId == "sublist_out_select") {
      ...
      if (selectVal_out) {
        var collectedRow = vrecord.getCurrentSublistValue({
          sublistId: "custpage_sublist_out",
          fieldId: "sublist_out_trans_payment_balance",
        });
      } else {
        var collectedRow = '';
      }
      ...
      console.log('selectVal_out', selectVal_out);
    ...
    if (context.sublistId == "custpage_sublist_out" && context.fieldId == "sublist_out_trans_transfer_amount") {
      ...
      console.log('collectedNow', collectedNow);
      if (changeVal_out) {
  ...
  function refreshlist(custpage_customer, filterItem, filterItemIn, filterItemRecord, trans_date_from, trans_date_to, vrecord) {
    try {
      ...
      const formula_account = "case when {salesorder}=' ' then {applyingtransaction.number} else {number} end";
      if (filterItem){
        allPaymentData.filters.push(
          search.createFilter({
            name: "FORMULATEXT",
     ...
     if(filterItemIn){
      ...
      if (typeof allPaymentData !== "undefined" && typeof vrecord !== "undefined") {
        for (var i in allPaymentData) {
          ...
          var totalCollected = py.getValue(allPaymentDataSet.columns[11]);
          var paymentBalance = py.getValue(allPaymentDataSet.columns[13]);
          var invDesc = py.getValue(allPaymentDataSet.columns[14])
          ...
          //   vrecord.setCurrentSublistValue({
          //     sublistId: "custpage_sublist_out",
          //     fieldId: "sublist_out_trans_invoice_desc",
          ...
          if (totalCollected){
            vrecord.setCurrentSublistValue({
              sublistId: "custpage_sublist_out",
              fieldId: "sublist_out_trans_total_collected",
