...
function(render, search, record, log, file, http, config, format, email, runtime) {
    ...
    function onRequest(context) {
        ...
        xml += "</macrolist>";
        xml += "</head>"
        xml += "<body font-size='10' style='font-family: Tahoma,sans-serif;height: 34cm; width: 48cm;' header='nlheader' header-height='" + headerHeight + "' footer='nlfooter' footer-height='3%'>";
        xml += body;
        xml += "\n</body>\n</pdf>";
        ...
        if(lineCount > 0){
            ...
            for(var index = 0; index < lineCount; index++){
                ...
                if(isBiaya === true){
                    ...
                    var transType =  recProject.getSublistText({
                        sublistId : 'recmachcustrecord_pcd_id',
                        fieldId : 'custrecord_pcd_type',
                        line : index
                    })
            ...
            hitung_baris += 3;
            var baris_perhal = hitung_baris
            var halaman = 0;
            var noMer = 0
            var totalBiayaProject = 0
            for (var tyPeBiaya in groupedData) {
                if (groupedData.hasOwnProperty(tyPeBiaya)) {
                    var rowspan = groupedData[tyPeBiaya].length;
                    log.debug('rowspan', rowspan);
                    var sisa_rowspan = rowspan
                    var sisa_baris = 0;
                    noMer++
                    log.debug('sisa_rowspan before', sisa_rowspan);
                    log.debug('hitung baris', hitung_baris)
                    if(hitung_baris <= 44){
                        halaman = 1
                        while((sisa_rowspan + baris_perhal) > 44){
                            sisa_rowspan --
                        }
                    }else{
                        halaman ++
                        while((sisa_rowspan + baris_perhal) > 45){
                            sisa_rowspan --
                        }
                    }
                    log.debug('sisa_rowspan after', sisa_rowspan);
                    log.debug('halaman', halaman)
                    for (var i = 0; i < groupedData[tyPeBiaya].length; i++) {
                        
                        hitung_baris ++
                        baris_perhal ++
                        var data = groupedData[tyPeBiaya][i];
                        var tyPeBiaya = data.tyPeBiaya
                        var transType = data.transType
                        log.debug('transType', transType);
                        var keterangan = data.keterangan
                        var estUnit = data.estUnit
                        var estNominal = data.estNominal
                        totalBiayaProject += Number(estNominal)
                        ...
                        body += "<tr>";
                        log.debug('baris_perhal', baris_perhal);
                        if(hitung_baris <= 44 || baris_perhal <= 45){
                            if(i < 1){
                                body += "<td style='border: 1px solid black; border-right:none; align:left; vertical-align: middle;' rowspan='"+sisa_rowspan+"'>"+noMer +" . "+transType+"</td>";
                            }else if((baris_perhal == 1 && halaman == 1) || (baris_perhal == 1 && halaman > 1)){
                                var hitungan = Number(rowspan) - Number(sisa_rowspan)
                                log.debug('hitungan', hitungan)
                                if(Number(rowspan) - Number(sisa_rowspan) > 45){
                                    var batas_rowspan = 45
                                    rowspan -= 45
                                    body += "<td style='border: 1px solid black; border-right:none; align:left; vertical-align: middle;' rowspan='"+batas_rowspan+"'>"+noMer +" . "+transType+"</td>";
                                }else{
                                    batas_rowspan = Number(rowspan) - Number(sisa_rowspan)
                                    log.debug('batasRowspan', batas_rowspan)
                                    log.debug('rowspan min', rowspan)
                                    log.debug('sisa rowspan min', sisa_rowspan)
                        ...
                        body += "<td style='border: 1px solid black; border-right:none; align:left;'>"+keterangan+"</td>";
                        body += "<td style='border: 1px solid black; border-right:none; align:right;'>"+estUnit+"</td>";
                        body += "<td style='border: 1px solid black; border-right:none; align:right;'>Rp.</td>";
                        body += "<td style='border: 1px solid black; border-left:none; border-right:none; align:right;'>"+formatNumber(estNominal)+"</td>";
                        body += "<td style='border: 1px solid black; border-right:none; align:right;'>"+actUnit+"</td>";
                        body += "<td style='border: 1px solid black; border-right:none; align:right;'>Rp.</td>";
                        body += "<td style='border: 1px solid black; border-left:none; border-right:none; align:right;'>"+formatNumber(actNominal)+"</td>";
                        body += "<td style='border: 1px solid black;  border-right:none; align:right;'>Rp.</td>";
                        body += "<td style='border: 1px solid black; border-left:none; align:right;'>"+formatNumber(selisih)+"</td>";
                        body += "<td style='border: 1px solid black; border-left:none; align:right;'>"+ketLine+"</td>";
                        body += "<td style='border: 1px solid black; border-left:none; align:right;'>"+persetujuan+"</td>";
                        body += "</tr>";

                        sisa_baris = baris_perhal
                        
                        if(halaman == 1){
                            baris_perhal = (baris_perhal == 44 ? 0 : baris_perhal);
                        }else{
                            baris_perhal = (baris_perhal == 45 ? 0 : baris_perhal);
                        }
                ...
                if(sisa_baris > 44){
                    for(var i = sisa_baris; i <= 45; i++){
                        body += "<tr style='background-color:white'>";
                        body += "<td colspan='12'></td>";
                        body += "</tr>";
                    }
                }
            }
            var selisiBiaya = Number(totalEstNominal) - Number(totalBiayaProject)
            if(selisiBiaya == 0){
                selisiBiaya = '-'
            }
            
            body += "<tr style='background-color:white'>";
            body += "<td colspan='2' style='border: 1px solid black; align:left; font-weight:bold'>TOTAL BIAYA PROJECT</td>";
            body += "<td style='border: 1px solid black; border-right:none; border-left:none; align:right; font-weight:bold; vertical-align: middle;' colspan='2'>Rp.</td>";
            body += "<td style='border: 1px solid black; border-left:none; border-right:none; align:right; font-weight:bold; vertical-align: middle;'>"+formatNumber(totalBiayaProject)+"</td>";
            body += "<td style='border: 1px solid black; align:left; font-weight:bold; vertical-align: middle;' colspan='7'></td>";
            body += "</tr>";

            body += "<tr style='background-color:white'>";
            body += "<td colspan='2' style='border: 1px solid black; align:left; font-weight:bold'>TOTAL SELISIH PENDAPATAN & BIAYA</td>";
            body += "<td style='border: 1px solid black; border-right:none; border-left:none; align:right; font-weight:bold; vertical-align: middle;' colspan='2'>Rp.</td>";
            body += "<td style='border: 1px solid black; border-left:none; border-right:none; align:right; font-weight:bold; vertical-align: middle;'>"+formatNumber(selisiBiaya)+"</td>";
            body += "<td style='border: 1px solid black; align:left; font-weight:bold; vertical-align: middle;' colspan='7'></td>";
            body += "</tr>";
            return body;
        }
