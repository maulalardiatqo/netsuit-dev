 ...
 */
 ...
 function(search,currentRecord,query,record,format, dialog, runtime) {
  ...
  function MarkAll(flag) {
	  ...
	  var count = rec.getLineCount({sublistId: 'sublist'});
	  console.log("count", count);
 	  for (var idx = 0; idx<count; idx++) {
		try	{
			rec.selectLine({sublistId: 'sublist',
								line: idx});
  ...
  function pageInit(context) {
	...
	rec.setValue({fieldId: 'custpage_subsidiary',value: dfltSubsidiary});
  }
  function validate(context){
	...
	RfqData = RfqDataSet.getRange({start:0,end:1000});
	if(RfqData.length>0){
		...
		hasPOLink = 0;
		var lineCountRFQAFC = loadRecordRFQItem.getLineCount({sublistId :'recmachcustrecord_abj_rfq_item_rfq'});
		...
		if(RfqDataPOLink || RfqDataPOLink != ''){
			hasPOLink++;
		}
		if(hasPOLink > 0){
  ...
  function saveRecord(context) {
	  ...
	  var count = rec.getLineCount({sublistId: 'sublist'});
 	  for (var idx = 0; idx < count; idx++) {
		...
		if (selectVal && (!potype)) {
			var alert_cant_empty = {title: 'PO Type Validation', message: "Please choose PO Type"};
  ...
  function fieldChanged(context) {
	    ...
	    if ((context.sublistId == 'sublist') && (context.fieldId == 'sublist_select')) {
			...
			console.log('totalcount',totalcount);
	 		vrecord.setValue({fieldId: 'custpage_line_count',value: totalcount,ignoreFieldChange: true});
	    }
  }
  ...
  function refreshlist(Rfq_no,vsubsidiary,vlocation,vclass,vdepartment,Rfq_type,
	...
	try
	{
		...
		var countsublist = vrecord.getLineCount({sublistId: 'sublist'});
		...
		while (countsublist>0) {	
			...
			countsublist = vrecord.getLineCount({sublistId: 'sublist'});
		}
		vrecord.setValue({fieldId: 'custpage_line_count',value: 0,ignoreFieldChange: true});
		...
		for (var idxtype = 0; idxtype < 2; idxtype++) { 
			...
			if (vsubsidiary) {
				...
				{name: 'custrecord_abj_rfq_subsidiary',operator: search.Operator.IS,values: vsubsidiary},
				));
			};
			...
			if (Rfq_type) {
				console.log('Rfq_type',Rfq_type);
				RfqData.filters.push(search.createFilter(
				{name: 'custrecord_abj_rfq_type',operator: search.Operator.IS,values: Rfq_type},
				));
			};
			...
			if (Rfq_date_from) {
				...
				RfqData.filters.push(search.createFilter(
				{name: 'custrecord_abj_rfq_date',operator: search.Operator.ONORAFTER,values: Rfq_date_from},
				));
			};
			...
			if (Rfq_date_to) {
				...
				RfqData.filters.push(search.createFilter(
				{name: 'custrecord_abj_rfq_date',operator: search.Operator.ONORBEFORE,values: Rfq_date_to},
				));
			};
			...
			for (var i in RfqData) {
				...
				RfqDate = format.parse({value:RfqDate, type: format.Type.DATE});
				...
				if (item_dlvr_Date) 
					item_dlvr_Date = format.parse({value:item_dlvr_Date, type: format.Type.DATE});
				...
				if (pr_receive_date) 
					pr_receive_date = format.parse({value:pr_receive_date, type: format.Type.DATE});
		...
		for (var i in data_rfq_to_Sort) {
			...
			vrecord.commitLine({sublistId: 'sublist', ignoreRecalc: true});
		}
