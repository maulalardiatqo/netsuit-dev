	...
	function onRequest(context) {
		...
		function update_rfq_with_PO_no(poId,listpolines,rfq_internal_id) {
			...
			{	
				...
				var dueDate = rfq_data_to_update.getValue('custrecord_abj_rfq_pr_duedate');
        ...
        if (contextRequest.method == 'GET') {
			...
			var rfq_data = record.load({
				type : 'customrecord_abj_rfq',
				id : rfq_internal_id,         
				isDynamic : true
			});
			log.debug('rfq_data', rfq_data);
			var dueDate = rfq_data.getValue('custrecord_abj_rfq_pr_duedate');
			if(dueDate){
			...
			function sysDate() {
			  ...
			  var days30 = sysDate();
			  days30 = format.parse({value:days30, type: format.Type.DATE});
			  log.debug('days30', days30);
			}
            ...
            for (var i = 0; i < count; i++) {
				try {
					...
					if ((vendor !== prev_vendor)||(po_type !== prev_po_type)) {
						...
						if(days30){
							PO.setValue({
								fieldId: 'custbody_value_date',
								value: days30, 
								ignoreFieldChange: false
							});
						}
						var dueDate = rfq_receive_Date
