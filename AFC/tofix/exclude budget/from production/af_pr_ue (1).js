  ...
  function(error, log, search, record, dialog, runtime) {
    ...
    function updateBudgetDetail(budgetdetail_rfq_exps, budgetdetail_rfq_expset,
      ...
      try {
        ...
        log.error("Error in Update Budget Detail", e.name + ' : ' + e.message);
      }
    }
    ...
    function beforeSubmit_rfq(context) {
      ...
      try {
        ...
        log.error('error in before submit RFQ', e.name + ': ' + e.message);
      }
    }
      ...
      try {
        if (((context.type === context.UserEventType.EDIT) || (context.type === context.UserEventType.DELETE))) {
          ...
          for (var counter = 0; counter < 2; counter++) {
            ...
            for (var i = 0; i < lineTotal; i++) {
              var excludeBudgetPO = rec.getSublistValue({
                sublistId: vsublistid,
                fieldId: 'custcol_abj_po_exclude_from_budget',
                line: i
              }) || false;
              log.debug("excludeBudget", excludeBudgetPO);
              if (!excludeBudgetPO) {
                var Department = rec.getSublistValue({
                  sublistId: vsublistid,
                  fieldId: 'department',
        ...
        log.error('error in before submit pr po', e.name + ': ' + e.message);
      }
    ...
    function DeleteBudgetAmount(BudgetPeriod_Text, account, Amount, BudgetDetails, budgetdetailset) {
      ...
      try {
        ...
        log.error('error in delete budget amount', e.name + ': ' + e.message);
      }
    }
    ...
    function afterSubmit_rfq(context) {
      ...
      var BudgetPeriod = rec.getValue('custrecord_abj_budperiod_tran');
      var Subsidiary = rec.getValue('custrecord_abj_rfq_subsidiary');
      var BudgetPeriodText = months[BudgetPeriod].toLowerCase();
      ...
      try {
        ...
        for (var counter = 0; counter < 2; counter++) {
          ...
          for (var i = 0; i < lineTotal; i++) {
            ...
            if (!excludeBudget) {
              ...
              account_text = rec.getSublistValue({
                sublistId: vsublistid,
                fieldId: fieldacct,
                line: i
              });
        ...
        log.error("Error in After Submit RFQ", e.name + ' : ' + e.message);
      }
    }
    ...
    function afterSubmit_pr_po(context) {
      ...
      try {
        ...
        if (rectype == 'purchaserequisition') {
          ...
          var pr_approval_status = rec.getValue('approvalstatus');
          if (pr_approval_status == '2') {
            ...
            let rfqNoCheck = 0;
            let totalItems = pr_data_to_update.getLineCount({
              sublistId: "item",
            });
            let totalExpense = pr_data_to_update.getLineCount({
              sublistId: "expense",
            });
            let totalLines = parseInt(totalItems) + parseInt(totalExpense);

            for (var i = 0; i < totalItems; i++) {
              let rfqCheck = rec.getSublistValue({
                sublistId: "item",
                fieldId: "custcolafc_pr_rfq_no",
                line: i,
              });
        ...
        log.error("Error in After Submit PR_PO", e.name + ' : ' + e.message);
      }
    }
