...
define(['N/currentRecord', 'N/record', 'N/search', 'N/runtime', 'N/ui/dialog', ],
  function(currentRecord, record, search, runtime, dialog) {
    ...
    function fieldChanged(context) {
      try {
        ...
        if (rectype == "purchaserequisition") {
          ...
          if(context.fieldId == 'duedate'){
            var today = new Date();
            if(rec.getValue('duedate') < today){
              window.alert('Please enter a delivery date larger than today');
              rec.setValue({fieldId : 'duedate', value: '', ignoreFieldChange : true});
            }
          }
          
          if(context.fieldId == 'date'){
            var today = new Date();
            if(rec.getValue('date') < today){
              window.alert('Please enter a PR date larger than today');
              rec.setValue({fieldId : 'date', value: '', ignoreFieldChange : true});
            }
          }
    ...
    function saveRecord_pr_po(context) {
      ...
      try {
        if (ispurchaseorder || (ispurchaserequisition && isnew)) {
          log.debug(rectype);
