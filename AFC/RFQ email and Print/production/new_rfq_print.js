      ...
      for (var i = 0; i < vendor_line_count; i++) {
        try {
          ...
          if (!isemailunawarded) {
            ...
            header += "<tr>";
            header += "<td class='tg-headerlogo' style='width:15%;vertical-align:center;'><img src='" + urlLogo +
              '\' width="80px" height="45px" ></img> </td>';
            ...
            body += " <th class='tg-headershipto' style='width:15%'>Unit Price</th>"
            body += " <th class='tg-headershipto' style='width:20%'>Total Price</th>"
            body += "</tr>";
            body += "</thead>";
            body += "<tbody>";
            ...
            xml += "</macrolist>";
            xml += "</head>"
            xml += "<body font-size='10' style='font-family: Tahoma,sans-serif;height: 29.7cm; width: 21cm;'  header='nlheader' header-height='" + headerHeight + "' footer='nlfooter' footer-height='12%'>";
            xml += body;
            xml += "\n</body>\n</pdf>";
            xml = xml.replace(/ & /g, ' &amp; ');
