  ...
  function onRequest(context) {
    ...
    for (var counter = 0; counter < 2; counter++) {
      ...
      for (var i = 0; i < lineTotal; i++) {
        ...
        //   .create({
        //     type: "item",
        //     columns: ["internalid","lastpurchaseprice", "custitem_abj_lpp_non_inv", "type"],
        //     filters: [
        //       {
        //         name: "internalid",
        ...
        //   });
          ...
          var itemFromSearch = search.load({
            id : "customsearch_abj_lpp_quot_summ"
          });
          ...
          itemFromSearch.filters.push(myFilter);
        
          var results = itemFromSearch.run();
          var itemFromSearch = results.getRange({
              start: 0,
              end: 5
          });
        var lastPurchasePrice = 0;
        if (itemFromSearch.length > 0){
          var itemType = itemFromSearch[0].getValue(results.columns[1]);
          log.debug('itemType', itemType);
          var toLastPurchaseif = itemFromSearch[0].getValue(results.columns[2]);
          log.debug('toLastPurchaseif', toLastPurchaseif);
          var toLastPurchaseEls = itemFromSearch[0].getValue(results.columns[3]);
          log.debug('toLastPurchaseEls', toLastPurchaseEls);
          if(itemType == "NonInvtPart" || itemType == "Service"){
            lastPurchasePrice = toLastPurchaseif;
            log.debug('lastPurchasePrice if', lastPurchasePrice)
          }else{
            lastPurchasePrice = toLastPurchaseEls;
            log.debug('lastPurchasePrice else', lastPurchasePrice)
          }
        }
