    ...
    function (render, search, record, log, file, http, config, format, email, runtime) {
        try {
            ...
            function onRequest(context) {
                ...
                style += ".tg .tg-img-logo{width:150px; height:37px; object-vit:cover;}"; // added by Kurnia
            ...
            function getLine2(context, allDataItem) {
                if (allDataItem.length > 0) {
                    ...
                    for (var index = 0; index < allDataItem.length; index++) {
                        ...
                        log.debug('unitConvertion', unitConvertion)
                        // 🔁 Loop per LOT untuk menampilkan beberapa baris
                        var unit
                        var totalqty = 0
                        allLot.forEach(function (lotData) {
                            nomor++;
                            var konversi = 0;
                            if (unitConvertion) {
                                konversi = Number(lotData.qtyLot) * Number(unitConvertion);
                            }
                            var qtyLot = Number(lotData.qtyLot) / Number(unitConvertion);
                            totalqty = Number(totalqty) + Number(lotData.qtyLot)
                            unit = satuan[satuan.length - 1]
                            body += "<tr>";
                            body += "<td style=''>" + nomor + "</td>";
                            body += "<td style='white-space: nowrap; text-align: center;'>" + escapeXmlSymbols(item) + "</td>";
                            body += "<td style=''>" + (escapeXmlSymbols(description) || escapeXmlSymbols(itemDesc)) + "<br/>" + escapeXmlSymbols(locationLine) + "</td>";
                            body += "<td style=''>" + escapeXmlSymbols(lotData.lot) + "</td>";
                            body += "<td style=''>" + units + "</td>";
                            body += "<td style=''>" + qtyLot+ "</td>";
                            body += "<td>" + lotData.qtyLot + " " +satuan[satuan.length - 1]+ "</td>";
                            body += "</tr>";
                        });
                        body += "<tr>";
                        body += "<td style=''></td>";
                        body += "<td style='white-space: nowrap; text-align: center;'></td>";
                        body += "<td style=''></td>";
                        body += "<td style=''></td>";
                        body += "<td style=''></td>";
                        body += "<td style=''></td>";
                        body += "<td>" + totalqty + " " +unit+ "</td>";
                        body += "</tr>";
                    }
                    return body;
                }
