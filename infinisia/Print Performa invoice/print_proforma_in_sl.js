  ...
  try {
    ...
    function onRequest(context) {
      ...
      var poSavedLoad = search.load({
        id: "customsearch_so_proforma_inv",
      });
      if(recid){
      ...
      var trandId = invRec.getValue({
        name: "custbodyiss_proforma_invoice_num"
      });
      ...
      var bankName = invRec.getValue({
        name: "custbody_iss_inv_branch_name"
      });
      var bankNumber = invRec.getValue({
        name: "custbody_iss_inv_bank_number"
      });
      var accName = invRec.getValue({
        name: "custbody_iss_inv_account_name"
      })
      if (idCust) {
      ...
      var itemSearch =  search.load({
        id: "customsearch_so_for_proforma_inv_line",
      });
      if(recid){
      ...
      if (itemCount.length > 0) {
        for (var index = 0; index < itemCount.length; index++) {
          ...
          var namaBarang = poRecord.getText({
              name: "item"
          });
          var rate = poRecord.getValue({
            name: "rate"
          });
          var unit = poRecord.getValue({
            name: "unit"
          });
      ...
      body += "<tr>";
      body += "<td style='font-size:18px; align:center; font-weight:bold;'>PROFORMA INVOICE</td>";
      //   body += "<td>Tracking Code :</td>";
      //   body += "<td style='align:right; font-weight:bold'>ATRY</td>";
      body += "</tr>";

      body += "<tr>";
      body += "<td style='font-size:18px; align:center; font-weight:bold;'></td>";
      ...
      body += "<tr>";
      body += "<td style='font-weight:bold; padding-top: 20px;'>Proforma Invoice #</td>";
      body += "<td style='font-weight:bold; padding-top: 20px;'>" + trandId + "</td>";
    ...
    function getGL(recId) {
      ...
      var invoiceSearchObj = search.create({
        type: "invoice",
        filters: [["type", "anyof", "CustInvc"], "AND", ["internalid", "anyof", recId]],
        columns: [search.createColumn({ name: "creditamount", label: "Amount (Credit)" }), search.createColumn({ name: "debitamount", label: "Amount (Debit)" }), search.createColumn({ name: "account", label: "Account" }), search.createColumn({ name: "memo", label: "Memo" })],
      });
      ...
      invoiceSearchObj.run().each(function (result) {
        ...
        if (account) {
          var accountSearchObj = search.create({
            type: "account",
            filters: [["internalid", "anyof", account]],
            columns: [
              search.createColumn({
                name: "name",
