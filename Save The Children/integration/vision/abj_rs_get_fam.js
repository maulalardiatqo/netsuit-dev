 ...
 * **Copyright (c) 2025 ABJ Cloud Solutions, Inc.
 * @Client        :  Suy Sing
 * @Script Name   :  - SSCC | RL | Get ITEMS INVENTORY
 * @script File   :  abj_rl_get_item_inventory.js
        ...
        try{
            ...
            page.data.forEach((result) => {
                const row = {
                    ...
                    serialNumber:     result.getValue({ name: 'custrecord_assetserialno' }),
                    assetCareTaker :  {
                    ...
                    assetType : {
                        assetTypeId:      result.getValue({ name: 'custrecord_assettype' }),
                        assetTypeName:    result.getText({ name: 'custrecord_assettype' }),
                    },
                    
                    originalCost:     result.getValue({ name: 'custrecord_assetcost' }),
                    currentCost:      result.getValue({ name: 'custrecord_assetcurrentcost' }),
                    depreciation:     result.getValue({ name: 'custrecord_assetaccmethod' }),
                    lifetime:         result.getValue({ name: 'custrecord_assetlifetime' }),
                    status : {
                       statusId :           result.getValue({ name: 'custrecord_assetstatus' }),
                       statusName :           result.getText({ name: 'custrecord_assetstatus' }),
                    },
                    componentOf : {
                        componentOfId : result.getValue({ name: 'custrecord_componentof' }),
                        componentOfName : result.getText({ name: 'custrecord_componentof' }),
                    },      
                    businessUnit : {
                        businessUnitId:    result.getValue({ name: 'cseg1' }),
                        businessUnitName: result.getText({ name: 'cseg1' }),
                    },
                    
                    quantity:         result.getValue({ name: 'custrecord_ncfar_quantity' })
                };
