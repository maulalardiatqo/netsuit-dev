    ...
    function(render, search, record, log, file, http, config, format, email, runtime) {
        ...
        function onRequest(context) {
            ...
            // load PO
                ...
                // load vendor
                var location = poRecord.getText('location')
                ...
                if(countItem > 0){
                    ...
                    for (var i = 0; i < countItem; i++) {
                        ...
                        var amountTaxt = poRecord.getSublistValue({
                            sublistId : 'item',
                            fieldId : 'tax1amt',
                            line : i
                        })
                        ...
                        log.debug('taxrate', taxtRate);
                        if(taxtRate !== 0){
                            var taxtExist = false;
                            for (var j = 0; j < itemTaxRate.length; j++) {
                                if (itemTaxRate[j] === taxtRate) {
                                    taxtExist = true;
                                    break;
                                }
                            }
                            ...
                            if (!taxtExist) {
                                itemTaxRate.push(taxtRate);
                            }
                        }
                        taxtotal += Number(amountTaxt);
                        if(whTaxCodeI){
                ...
                log.debug('itemTaxRate', itemTaxRate);
                ...
                if (Array.isArray(itemTaxRate) && itemTaxRate.length > 0) {
                    integerPart = itemTaxRate[0];

                    if (itemTaxRate.length > 1) {
                        decimalPart = itemTaxRate[1];
                    }
                }

                var displayString = "VAT " + integerPart;
                ...
                style += ".tg .tg-img-logo{width:150px; height:140px; object-vit:cover;}";
                // if(subsidiari == 1){
                ...
                body += ""+ POdate + "</p>"
                body += "<p class='tg-headerrow_legalName'> Location : "+ location + "</p>"
                ...
                body += "<td class='tg-head_body' style='width:30%'> ITEM </td>"
                ...
                log.debug('itemTaxtRate', itemTaxRate);
                body += "<td class='tg-f_body'>"+displayString+"</td>"
