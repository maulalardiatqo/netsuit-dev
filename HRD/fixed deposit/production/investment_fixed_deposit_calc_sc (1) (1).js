...
define(["N/search", "N/record", "N/runtime", "N/format"], function(
  ...
  function execute(context) {
    ...
    // log.debug("accured", accured);
    // log.debug("upon_maturity", upon_maturity);
    ...
    // var todayDateYear = getUTCMidnight('2022-12-31');
    var todayDateYear = getCurrentUTCMidnight();
    log.debug("todayDateYear", todayDateYear);
    // var todayDateYear = todayyy.setHours(18, 0, 0);
    ...
    var LastDateInYear = new Date(todayDateYear.getFullYear(), 11, 31);
    var yesterdayDate = new Date(todayDateYear.setDate(todayDateYear.getDate() - 1));
    ...
    var lastDateFormatCheck = format_date_for_save_search(LastDateInYear);
    var yesterdayFormatCheck = format_date_for_save_search(yesterdayDate);
    ...
    log.debug("date", {
      ...
      LastDateInYear: LastDateInYear,
      yesterdayDate: yesterdayDate
    });

    // var z6 = FirstDateInYear; //new Date("1/1/2021");
    // log.debug("ReportAsofDate", z7);
    // log.debug("FirstDateInYear", z6);
    // var z14 = days(z7, z6);
    ...
    data_invst_1.forEach(function(result) {
      try {
        ...
        var j_14 = toDateFormat(j_14);
        var k_14 = format_date_for_save_search(
          investrecord.getValue(
            "custrecord_sol_invtr_fd_maturity_date"
          )
        );
        var k_14 = toDateFormat(k_14);
        log.debug("Start Date & Maturity", {
          start_date: j_14,
          maturity: k_14
        });
        var j_14_formated = getUTCMidnight(j_14);
        var k_14_formated = getUTCMidnight(k_14);
        ...
        var calc_x14 = days(todayDateYear, j_14_formated);
        var calc_x14 = parseFloat(calc_x14);
        // var calc_v14 = calc_x14 < calc_z14 ? calc_x14 : calc_z14;
        // var calc_v14 = days(new Date(), FirstDateInYear);
        var j_14_row = new Date(j_14_formated);
        var j_14_year = j_14_row.getFullYear();
        var todayYear = todayDateYear.getFullYear();
        // log.debug("j_14_row", j_14_row);
        // log.debug("j_14_year", {
        //   j_14_year: j_14_year,
        //   todayYear: todayYear
        // })
        if (j_14_year < todayYear) {
          var fromDate = YesterdayFirstDateInYear;
        } else {
          var fromDate = j_14_formated;
        }
        log.debug("Total Days Up", {
          todayDateYear: todayDateYear,
          fromDate: fromDate,
          YesterdayFirstDateInYear: YesterdayFirstDateInYear,
          j_14_formated: j_14_formated
        });

        var calc_v14 = days(todayDateYear, fromDate);
        var md_sd = days(k_14_formated, j_14_formated);
        var f_14 =
          investrecord.getValue(
        ...
        if (todayFormatCheck == firstDateFormatCheck) {
          ...
          var calc_p14 =
            parseFloat(f_14) * (parseFloat(h_14) / 100) * (parseFloat(calc_v14) /
              365);
        }
        // var calc_p14 =
        //   parseFloat(f_14) * (parseFloat(h_14) / 100) * (parseFloat(calc_v14) /
        ...
        var calc_q14 =
          ...
          365;
        var calc_w14 = days(new Date(k_14_formated), todayDateYear);
        // var calc_w14 = calc_w14 < 0 ? 0 : calc_w14;
        // var tomorrow = new Date(todayDateYear);
        // var tomorrowDate = tomorrow.setDate(tomorrow.getUTCDate() + 1);
        // var yesterdayDate = tomorrow.setDate(tomorrow.getUTCDate() - 1);
        // var tomorrowDateCheck = format_date_for_save_search(tomorrowDate);
        // var yesterdayDateCheck = format_date_for_save_search(yesterdayDate);
        // log.debug("date check maturity", {
        //   maturity_date: k_14,
        //   tomorrowDateCheck: tomorrowDateCheck,
        //   yesterdayDateCheck: yesterdayDateCheck
        // });
        // if (k_14 == tomorrowDateCheck) {
        //   var calc_w14 = 1;
        // } else if (k_14 == yesterdayDateCheck) {
        //   var calc_w14 = -1;
        // }
        var calc_r14 =
        ...
        // var days_t4_j14 = days(yearLast, new Date(j_14_formated));
        ...
        //   lastDateFormatCheck: lastDateFormatCheck
        // })
        if (todayFormatCheck == firstDateFormatCheck) {
          log.debug("today is end date", true);
          var calc_t14 = accuredInteres;
        } else {
          var calc_t14 = t_14;
        }

        // var lt_14 = parseFloat(l_14) - parseFloat(calc_t14);
        // log.debug("calc_q14", calc_q14);
        ...
        if (k_14 <= toDateFormat(todayFormatCheck)) {
          var calc_l14 = (parseFloat(calc_y14) / 365) * (parseFloat(h_14) / 100) * parseFloat(n_14);
        } else {
          var calc_l14 = l_14;
        }
        ...
        // var calc_u14 = lt_14;
        var calc_u14 = lt_14 < qt_14 ? qt_14 : lt_14;
        // log.debug("qt_14", qt_14);
        // log.debug("lt_14", lt_14);
        ...
        log.debug("Date Debug", {
          start_date: j_14_formated,
          maturity_date: k_14_formated,
          today_date: todayDateYear,
          total_days_up: calc_v14,
          upon_maturity: calc_w14,
          accured_current: calc_x14,
          total_days: calc_y14,
