  ...
  function execute(context) {
    ...
    data_invst_1.forEach(function(result) {
      try {
        ...
        var newValueForaccuredInterestCurrent = (diffInDays / 365) * (parseFloat(aiPrefiousFY)) * (parseFloat(getInterestProfitRate));
        

        if (todayFormatCheck == firstDateFormatCheck) {
          var accuredInterestCurrent = 0;
        } else {
          var accuredInterestCurrent = newValueForaccuredInterestCurrent
        ...
        //   'diffdays': diffInDays,
        //   'AI' : aiPrefiousFY,
        //   'profite rate': getInterestProfitRate,
        //   'newValue': newValueForaccuredInterestCurrent,
        //   'curValue' : accuredInterestCurrent
        // })
        var currDay = new Date();
        var year = currDay.getFullYear();
        var month = ("0" + (currDay.getMonth() + 1)).slice(-2);
        var day = ("0" + currDay.getDate()).slice(-2);
        ...
        var newValaccuredIterest = (diffCurrStart / 365) * (parseFloat(getInterestProfitRate)) * (parseFloat(aiPrefiousFY));
        var accuredInterest = newValaccuredIterest;
        ...
        log.debug({
          'diffCurstart' : diffCurrStart,
          'newValue' : newValaccuredIterest,
          'currVale' : accuredInterest,
          'name' : name,
          'maturity' : fdMaturityDate
        })
        ...
        var totalInterestOnMaturity = parseFloat(interestProfitEarnedCurrent) - parseFloat(accuredInterest);
        if (todayFormatCheck == firstDateFormatCheck) {
