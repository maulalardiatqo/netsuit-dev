...
define(['N/search', 'N/ui/message', 'N/currentRecord', 'N/record', 'N/ui/dialog', 'N/url', 'N/format'], function(search, message, currentRecord, record, dialog, url, format) {
  ...
  function generateNumber() {
    ...
    try {
      ...
      dialog.alert(error_dialog).then(() => {
        window.location.reload();
      });
      reject(e);
    }
  ...
  function PAGEFORM() {
    return url.resolveScript({
      scriptId: "customscript_void_custom_journal_sl",
      deploymentId: "customdeploy_void_custom_journal_sl",
      returnExternalUrl: false,
    });
  }

  function voidCustomJournal(context) {
    var id = currRecord.id;
    console.log("id", id);
    var openPageForm = PAGEFORM();
    openPageForm += "&postid=" + id;
    //console.log('open form', openPageForm);
    window.location.href = openPageForm;
  }

  function voidCustomJournal_OLD() {
      ...
      try {
        ...
        for (var i in glBillPaymentSearch) {
          ...
          if (i == 0) {
            ...
            console.log('acountDebit', accountDebit);
          }
        }
