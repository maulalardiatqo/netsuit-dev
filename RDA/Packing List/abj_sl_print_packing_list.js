         ...
         */
            (format, log, record, search, file, runtime, dateUtils) => {
                ...
                // Access user details
                    ...
                    var barangLineWithoutRemind = 0
                    var cekLine = 0
                    var cekLineBarang = 0
                    var page = 1;
                    for (let k = 0; k < itemCount; k++) {
                        let noInvoiceFaktur = itemFulfillTrx[k].getValue({name : 'tranid'})
                        let checkIfExist = tranIdPrintedCheck.find(tran => tran == noInvoiceFaktur);
                        if(!checkIfExist){
                            dataFaktur.push(noInvoiceFaktur)
                            tranIdPrintedCheck.push(noInvoiceFaktur)
                        }
                    }
                    log.debug('dataFaktur', dataFaktur)
                    ...
                    for (let i = 0; i < itemCount; i++) {
                        ...
                        if(line && findItem){
                            ...
                            if(cekLineBarang > 0){;
                                ...
                                if(i < itemCount -1){
                                    log.debug('bukan item terahir')
                                }else{
                                    log.debug('item terahir')
                                }
                            }
                        }
                    }
                    ...
                    let bodyBarang2 = `
                        <table width="100%" style="border-collapse: collapse;table-layout:auto;">
                                <thead>
                                    <tr>
                                        <td style="width:70px;" colspan="2">#Faktur</td>
                                        <td style="text-align:left; font-size:12px;" colspan="8">
                                            <p style="text-align:left">: ${dataHeader.faktur?.join(',')}</p>
                                        </td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td style="border: 1px solid black;border-left:0;border-right:0;align:left; width:50%;">Nama Barang <br/>Kemasan</td>
                    ...
                    xml += "</macrolist>";
                    xml += "</head>"
                    xml += "<body font-size='10' style='font-family: Tahoma,sans-serif;padding:8px;' header='nlheaderBarang' header-height='6cm' footer='nlfooter' footer-height='0'>";
                    xml += bodyBarang;
                    xml += "\n</body>\n</pdf>";
