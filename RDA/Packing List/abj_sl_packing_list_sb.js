                    ...
                    for (var i = 0; i < lineCount; i++) {
                        ...
                        if(fulfill == 'T'){
                            ...
                            if (subArea && area) {
                                ...
                                var subAreaSearch = search.create({
                                ...
                                var searchResult = subAreaSearch.run().getRange({ start: 0, end: 1000 });
                                for (var j = 0; j < searchResult.length; j++) {
                                    var validSubAreaId = searchResult[j].getValue({ name: 'internalid' });
                                    if (subArea == validSubAreaId) {
                                        isSubAreaValid = true;
                                        break;
                                    }
                                }
                                ...
                                if (!isSubAreaValid) {
                                    log.debug('SKIPPED', 'SubArea tidak valid di area ' + area + ', line ke-' + i);
                                    continue; // Langsung skip baris ini
                                }
                            
                                // Kalau valid, baru assign
                                subAreaSet = subArea;
                                areaSet = area;
                            
                            } else {
                                // Kalau tidak ada area atau subarea, bisa juga skip langsung
                                log.debug('SKIPPED', 'Area/SubArea kosong di line ke-' + i);
                                continue;
                            }
