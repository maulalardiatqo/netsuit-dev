...
define(['N/record', 'N/runtime', 'N/search'], function(record, runtime, search) {
    ...
    function getInputData(context) {
        ...
        var paramsArray = currentScript.getParameter({ name: 'custscript_params_array' });

        if (paramsArray) {
            try {
                return JSON.parse(paramsArray); 
            } catch (e) {
                log.error('Error parsing paramsArray', e);
            }
        } else {
            // fallback kalau masih ada param single
    ...
    function map(context) {
        ...
        log.debug('data', data)
        var startDate = formatToDDMMYYYY(data.startDate);
        var endDate = formatToDDMMYYYY(data.endDate);
