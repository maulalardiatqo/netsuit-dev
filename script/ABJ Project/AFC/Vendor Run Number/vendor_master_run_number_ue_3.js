  ...
  function afterSubmit(context) {
    try {
      ...
      if (context.type == context.UserEventType.CREATE || isEdit) {
        ...
        let vdrEntityID = rec.getValue('entityid');
        let vdrEntityIDText = rec.getText('entityid') || '';
        let vdrOwner = rec.getText('custentity_vdr_owner');
        let vdrGroup = rec.getValue('custentity_abj_vdr_grp');
        let isperson = rec.getValue('isperson');
        if (isEdit) {
        ...
        // if (isEdit &&
        //   (oldvdrOwner == vdrOwner) &&
        //   (oldvdrGroup == vdrGroup) &&
        //   (oldisperson == isperson)) {
        //   return true;
        // }

        let vdrCompName;
        if (isperson == 'T') {
          vdrCompName = rec.getValue('firstname') + ' ' + rec.getValue('middlename') + ' ' + rec.getValue('lastname');
        } else {
          vdrCompName = rec.getValue('companyname');
        }
        ...
        if (vdrGroup == 1) {
          ...
          vendorRunningNumbers.forEach(function(vendorRunningNumber) {
            ...
            // if (!vdrEntityIDText || ((vdrEntityIDText.charAt(0) != "P" && vdrEntityIDText.charAt(0) != "F" &&
            //       !vdrEntityIDText.includes("-")) ||
            //     (!vdrEntityIDText.includes("-")))) {
            //   var lastNumber = parseInt(vendorRunningNumber.getValue({
            //     name: 'custrecord_van_increment'
            //   }) || 0) + 1;
