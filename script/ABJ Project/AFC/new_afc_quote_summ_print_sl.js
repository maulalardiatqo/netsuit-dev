...
define([
  ...
  function onRequest(context) {
    ...
    style +=
      ".tg .tg-noborderitem{border-top: none;border-bottom: none;}";
    style +=
      ".tg .tg-noborderclass{border-bottom: none;}";
    style +=
      ".tg .tg-nobordersubtotal{border-top: none;}";
    style +=
    ...
    var body =
      '<table class=\'tg\' width="100%" style="margin-top:30px;">';
    body += "<tbody>";
    ...
    body += "<td style='width:70px;'>Last Purchase Price Item</td>";
    body += "<td style='width:70px;'>Estimated Value</td>";
    body += "</tr>";
    ...
    for (var idx in class_activity) {
      ...
      body += "<tr>";
      body += "<td class='tg-noborderclass'>" + class_text + " - " + activity_code_text + "</td>";
      ...
      for (var jdx in class_activity_item) {
        if (
          ...
          body += "<tr>";
          body += "<td class='tg-noborderitem'><p style='margin-left: 10px;'>" + item_text + "</p></td>";
          for (var vdx in vendor_list) {
            body += "<td class='tg-noborderitem' style=\"align: right;font-size:10px;\">";
      ...
      body += "<tr>";
      body += "<td class='tg-nobordersubtotal' style='font-weight: bold;'>Subtotal</td>";
      log.debug("SubTotal arr", SubTotal);
      for (var vdx in vendor_list) {
        body += "<td class='tg-nobordersubtotal' style=\"font-weight: bold;align: right;font-size:10px;\">";
        var vendor_to_check = vendor_list[vdx].value;
      ...
      for (var tdx in grandTotalAmountResult) {
        ...
        if (vendor_to_check == vendor_val) {
          if (grandTotalAmountResult[tdx].costSaving < 0) {
            var costSavingVal = "(" + formatCurrency(grandTotalAmountResult[tdx].costSaving || 0) + ")";
          } else {
            var costSavingVal = formatCurrency(grandTotalAmountResult[tdx].costSaving || 0);
          }
          body += "<td style=\"font-weight: bold;align: right;font-size:10px;\">" + costSavingVal + "</td>";
        }
      }
    }
    body += "</tr>";
      ...
      for (var tdx in grandTotalAmountResult) {
        ...
        if (vendor_to_check == vendor_val) {
          if (grandTotalAmountResult[tdx].costAvoided < 0) {
            var costAvoidedVal = "(" + formatCurrency(grandTotalAmountResult[tdx].costAvoided || 0) + ")";
          } else {
            var costAvoidedVal = formatCurrency(grandTotalAmountResult[tdx].costAvoided || 0);
          }
          body += "<td style=\"font-weight: bold;align: right;font-size:10px;\">" + costAvoidedVal + "</td>";
        }
      }
    }
    body += "</tr>";
