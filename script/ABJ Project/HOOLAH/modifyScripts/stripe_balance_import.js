...
define([
  ...
  "N/runtime",
  "SuiteScripts/moment",
], function (record, https, search, format, runtime, moment) {
  function getLastObj() {
  ...
  function getObj(recid) {
    var payout_search = search.create({
      type: "customrecord_stripe_balance_transactions",
      filters: [["custrecord_payout_id", "is", recid]],
      columns: ["custrecord_payout_id"],
    });
  ...
  function getInputData() {
    ...
    if (starting_after) payouts_url += "&starting_after=" + starting_after;
    ...
    var dataFiltered = data.filter((element) => {
      return element.available_on > 1648760234;
    });

    log.debug("data filter", dataFiltered.length);
