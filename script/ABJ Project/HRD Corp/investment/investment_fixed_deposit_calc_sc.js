...
define(["N/search", "N/record", "N/runtime", "N/format"], function(
  ...
  function execute(context) {
    ...
    try {
      ...
      var total_execute = 0;
      log.debug("data_invst_1", data_invst_1.length);
      data_invst_1.forEach(function(result) {
        ...
        // log.debug("interest_profit_daily", interest_profit_daily);
        var calc_l14 = parseFloat(n_14) * interest_profit_daily;
        // log.debug("calc_l14", calc_l14);
        var lt_14 = parseFloat(calc_l14) - parseFloat(calc_t14);
        // log.debug("lt_14", lt_14);
        var calc_u14 = lt_14;
        // log.debug("calc_u14", calc_u14);
        // investrecord.setValue({
        // 	fieldId: "custrecord_sol_invtr_fd_weghted_avg_rate",
        // 	value: calc_g14.toFixed(2),
        // 	ignoreFieldChange: true,
        // });
        var step_1 = investrecord.setValue({
          fieldId: "custrecord_sol_invtr_fd_weghted_avg_rate",
          value: calc_i14.toFixed(2) || 0,
          ignoreFieldChange: true,
        });
        log.debug("step_1", step_1);
        var step_2 = investrecord.setValue({
          fieldId: "custrecord_sol_invtr_fd_intst_prfit_fy",
          value: calc_l14.toFixed(2) || 0,
          ignoreFieldChange: true,
        });
        log.debug("step_2", step_2);
        var step_3 = investrecord.setValue({
          fieldId: "custrecord_sol_invtr_fd_acrued_inst_fy",
          value: calc_p14.toFixed(2) || 0,
          ignoreFieldChange: true,
        });
        log.debug("step_3", step_3);
        var step_4 = investrecord.setValue({
          fieldId: "custrecord_sol_invtr_fd_accured_interest",
          value: calc_q14.toFixed(2) || 0,
          ignoreFieldChange: true,
        });
        log.debug("step_4", step_4);
        var step_5 = investrecord.setValue({
          fieldId: "custrecord_sol_invtr_fd_intst_maturty_dt",
          value: calc_r14.toFixed(2) || 0,
          ignoreFieldChange: true,
        });
        log.debug("step_5", step_5);
        var step_6 = investrecord.setValue({
          fieldId: "custrecord_sol_invtr_fd_proft_maturty_dt",
          value: calc_s14.toFixed(2) || 0,
          ignoreFieldChange: true,
        });
        log.debug("step_6", step_6);
        var step_7 = investrecord.setValue({
          fieldId: "custrecord_sol_invtr_fd_proft_last_yr",
          value: calc_t14.toFixed(2) || 0,
          ignoreFieldChange: true,
        });
        log.debug("step_7", step_7);
        var step_8 = investrecord.setValue({
          fieldId: "custrecord_sol_invtr_fd_net_int_thisfy",
          value: calc_u14.toFixed(2) || 0,
          ignoreFieldChange: true,
        });
        log.debug("step_8", step_8);
        var step_9 = investrecord.setValue({
          fieldId: "custrecord_sol_invtr_fd_total_days_up",
          value: calc_v14 || 0,
          ignoreFieldChange: true,
        });
        log.debug("step_9", step_9);
        var step_10 = investrecord.setValue({
          fieldId: "custrecord_sol_invtr_fd_upon_maturity",
          value: calc_w14 || 0,
          ignoreFieldChange: true,
        });
        log.debug("step_10", step_10);
        var step_11 = investrecord.setValue({
          fieldId: "custrecord_sol_invtr_fd_accured_thisfy",
          value: calc_x14 || 0,
          ignoreFieldChange: true,
        });
        log.debug("step_11", step_11);
        var step_12 = investrecord.setValue({
          fieldId: "custrecord_sol_invtr_fd_total_days",
          value: calc_y14 || 0,
          ignoreFieldChange: true,
        });
        log.debug("step_12", step_12);
        var process_date = new Date();
        var step_13 = investrecord.setValue({
          fieldId: "custrecord_sol_invtr_fd_process_date",
          value: process_date,
          ignoreFieldChange: true,
        });
        log.debug("step_13", step_13);

        var investrecord_id = investrecord.save({
          enableSourcing: false,
          ignoreMandatoryFields: true,
        });
        log.debug("investrecord_id", investrecord_id);
