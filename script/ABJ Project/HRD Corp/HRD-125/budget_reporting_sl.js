  ...
  function onRequest(context) {
    ...
    if (contextRequest.method === "GET") {
      ...
      try {
        ...
        function getColoumnMonth(month) {
          switch (month) {
            case 'Jan':
              var getColoumn = 3;
              break;
        ...
        if (groupFilterSelected == 'ytd' || groupFilterSelected == 'mtd') {
          ...
          dataTrans.forEach(function(row) {
            ...
            var coloumn = getColoumnMonth(monthNow);
            if (groupFilterSelected == 'ytd') {
              var totalPlanned = 0;
              for (var i = coloumn; i >= 3; i--) {
                totalPlanned += parseFloat(row.getValue({
                  name: dataTransSet.columns[i]
                }));
              }
            } else {
              var totalPlanned = row.getValue({
                name: dataTransSet.columns[coloumn]
              });
            }
            ...
            var accountSearch = search.create({
              ...
              filters: [{
                name: 'custrecord_sol_budsub_gl_acc_grp_desc',
                operator: search.Operator.IS,
                values: groupDescVal
              }],
            })
            ...
            accountSearch.forEach(function(row0) {
              ...
              var subParentAccountText = row0.getText({
                name: accountSearchSet.columns[0]
              });
              log.debug("subParentAccountText", {
                subParentAccountText: subParentAccountText,
                subParentAccount: subParentAccount
              });
              for (var j in listUtilized) {
                ...
                if (accountUtilized == subParentAccount) {
                  ...
                  log.debug("same utilized", {
                    ...
                    accountUtilized: accountUtilized
                  });
                }
              }
              for (var k in listCommitted) {
                ...
                if (accountCommitted == subParentAccount) {
                  var amountAccountCommitted = listCommitted[j]?.amount || 0;
        ...
        if (listBudget.length <= 0) {
          ...
          var objXlsFile = file.create({
            name: "Budget Report " + groupFilterSelected.toUpperCase() + ".xls",
