...
define(["N/runtime", "N/log", "N/url", "N/currentRecord", "N/record", "N/ui/dialog"], function(
  ...
  function rfqPosting(context) {
    try {
      var id = currRecord.id;
      console.log("id", id);

      var dataTrans = record.load({
        type: "customrecord_sol_rfq",
        id: id,
        isDynamic: true
      });
      var entity = dataTrans.getValue("custrecord_sol_rfq_awarded");
      var entity_name = dataTrans.getText("custrecord_sol_rfq_awarded");
      var memo = dataTrans.getValue("altname");
      var department = dataTrans.getValue("custrecord_sol_rfq_department");
      var currency = dataTrans.getValue("custrecord_sol_rfq_currency");
      var requestor = dataTrans.getValue("custrecord_sol_rfq_requestor");
      ...
      rec_PR.setValue({
        fieldId: 'custbody_sol_rfq_awardedvendor',
        value: entity,
      });
      ...
      var lineTotal = dataTrans.getLineCount({
        sublistId: "recmachcustrecord_sol_rfq_link",
      });
      ...
      for (var i = 0; i < lineTotal; i++) {
        var bidder = dataTrans.getSublistValue({
          sublistId: "recmachcustrecord_sol_rfq_link",
          fieldId: "custrecord_sol_rfq_item_bidder",
          line: i,
        });
        if (bidder == entity) {
          totalItem++;
          var item = dataTrans.getSublistValue({
            sublistId: "recmachcustrecord_sol_rfq_link",
            fieldId: "custrecord_sol_rfq_item",
            line: i,
          });
          var quantity = dataTrans.getSublistValue({
            sublistId: "recmachcustrecord_sol_rfq_link",
            fieldId: "custrecord_sol_rfq_quantity_range",
            line: i,
          });
          var description = dataTrans.getSublistValue({
            sublistId: "recmachcustrecord_sol_rfq_link",
            fieldId: "custrecord_sol_rfq_description",
            line: i,
          });
          var rate = dataTrans.getSublistValue({
            sublistId: "recmachcustrecord_sol_rfq_link",
            fieldId: "custrecord_sol_rfq_price",
            line: i,
          });
          var amount = dataTrans.getSublistValue({
            sublistId: "recmachcustrecord_sol_rfq_link",
            fieldId: "custrecord_sol_rfq_line_total",
            line: i,
          });
          console.log("dataGet", {
            item: item,
      ...
      if (prId) {
        ...
        dataTrans.setValue({
          fieldId: "custrecord_sol_rfq_requisition",
          value: prId,
        });
        dataTrans.save({
          enableSourcing: true,
