/*******************************************************************************
 * @Client        :  HRDC
 * @Script Name   :  etris_rl_send_payment_transaction.js
 * @script Record :  - ETRIS RL | Send Payment Transaction
 * @Trigger Type  :  post
  ...
  function _post(context) {
    try {
      ...
      let folderId = scriptObj.getParameter({
        name: 'custscript_etris_rl_folder_id2'
      });
      ...
      let fileId = file.create({
        name: "Payment_Transaction_" + _date.getDate() + '_' + (_date.getMonth() + 1) + '_' + _date.getFullYear() + '_' + _date.getTime(),
      ...
      etRisJsonRecObj.setValue('custrecord_sol_etris_process', 3); //3. Payment Transaction
      ...
      etRisJsonRecObj.setValue('custrecord_sol_etris_process_name', context.levy_service_payment_tran.transaction.process_name);
      etRisJsonRecObj.setValue('custrecord_sol_etris_transaction_scheme', context.levy_service_payment_tran.transaction.scheme);
      etRisJsonRecObj.setValue('custrecord_sol_etris_isrefund', context.levy_service_payment_tran.transaction.refund);
      etRisJsonRecObj.setValue('custrecord_sol_etris_cancellation', context.levy_service_payment_tran.transaction.cancellation);
      var statusRecId = etRisJsonRecObj.save();
      log.debug("statusRecId", statusRecId);

      var jsonData = {
        'custbody_sol_supplier_type': context.levy_service_payment_tran.transaction.supplier_type,
        'custbody_sol_etris_supplier_id': context.levy_service_payment_tran.transaction.supplier_id,
        'custbody_sol_etris_claims_id': context.levy_service_payment_tran.transaction.claims_id,
        'custbody_sol_si_ref': context.levy_service_payment_tran.transaction.claims_id,
        'custbody_sol_etris_application_id_': context.levy_service_payment_tran.transaction.application_id,
        'custbody_sol_etris_scheme': context.levy_service_payment_tran.transaction.scheme,
        'custbody_sol_etris_process_name': context.levy_service_payment_tran.transaction.process_name,
        'custbody_sol_etris_transaction_desc_': context.levy_service_payment_tran.transaction.transaction_desc,
        'custbody_sol_etris_country_code': context.levy_service_payment_tran.transaction.country_code,
        'custbody_sol_etris_typeof_payment_': context.levy_service_payment_tran.transaction.typeof_payment,
        //'custbody_sol_etris_bank_swift_code' : context.levy_service_payment_tran.transaction.bank_swift_code,
        'custbody_sol_etris_bank_account_number': context.levy_service_payment_tran.transaction.bank_account_number,
        'custbody_sol_etris_approved_amount_': context.levy_service_payment_tran.transaction.approved_amount,
        'custbody_sol_etris_approved_date': context.levy_service_payment_tran.transaction.approved_date,
        'custbody_sol_etris_actual_payment_date': context.levy_service_payment_tran.transaction.actual_payment_date,
        'custbody_sol_etris_pay_month': context.levy_service_payment_tran.transaction.pay_month,
        'custbody_sol_etris_pay_year': context.levy_service_payment_tran.transaction.pay_year,
        'custbody_sol_etris_customer_type': context.levy_service_payment_tran.transaction.customer_type,
        'custbody_sol_etris_customer_id': context.levy_service_payment_tran.transaction.customer_id,
        'custbody_sol_etris_customer_name': context.levy_service_payment_tran.transaction.customer_name,
        'custbody_sol_etris_customer_desn': context.levy_service_payment_tran.transaction.customer_desn,
        'custbody_sol_etris_remarks_': context.levy_service_payment_tran.transaction.remarks,
        'custbody_sol_etris_refund': context.levy_service_payment_tran.transaction.refund,
        'custbody_sol_etris_cancellation_': context.levy_service_payment_tran.transaction.cancellation,
        'custbody_sol_etris_status_claims_id': context.levy_service_payment_tran.transaction.status_claims_id,
        'custbody_sol_etris_status_grant_id': context.levy_service_payment_tran.transaction.status_grant_id
      };
      log.debug("jsonData", JSON.stringify(jsonData));

      let eTrisMapperSearchObj = searchEtrisMapper(3, jsonData.custbody_sol_etris_process_name, jsonData.custbody_sol_etris_cancellation_, jsonData.custbody_sol_etris_refund, jsonData.custbody_sol_etris_scheme); //processName, isCancel, isrefund, scheme
      ...
      if (eTrisMapperSearchObj.length > 0) {
        ...
        var arAccount = 2922;
        var apAccount = 2821;
        var _trandate = (jsonData.custbody_sol_etris_approved_date).replaceAll('-', '/');
        log.debug('trandate', _trandate);
        var amount = jsonData.custbody_sol_etris_approved_amount_;
        ...
        if (invProcess) {
          if (customerSearchObj.length > 0) {
            ...
            if (invItemSearchObj.length > 0) {
              ...
              let invId = ETRIS_MODULE.createInvoice(customerId, _trandate, arAccount, _invItemId, amount, jsonData, departmentId, 3, isSST);
              inv_id = invId;
        ...
        if (custPaymentProcess) {
          if (customerSearchObj.length > 0) {
            ...
            if (invItemSearchObj.length > 0) {
              ...
              let invId = ETRIS_MODULE.createInvoice(customerId, _trandate, arAccount, _invItemId, amount, jsonData, departmentId, 3);
              inv_id = invId;
              ...
              if (cpSchemeSearchObj.length > 0 && invId) {
                ...
                let custPayID = ETRIS_MODULE.createCustomerPayment(invId, _trandate, arAccount, bankAccount, jsonData, departmentId, 3);
                cpay_id = custPayID;
                etrismapRecId = mappingRecId;
                _status = "SUCCESS";
        ...
        if (vendBillProcess) {
          if (vendorSearchObj.length > 0) {
            ...
            if (procItemSearchObj.length > 0) {
              ...
              let vendBillId = ETRIS_MODULE.createVendorBill(vendorId, _trandate, apAccount, procItemId, amount, jsonData, departmentId, 3);
              vb_id = vendBillId;
              etrismapRecId = mappingRecId;
              _status = "SUCCESS";
          ...
          if (vendorSearchObj.length > 0) {
            ...
            if (procItemSearchObj.length > 0) {
              ...
              if (schemeSearchObj.length > 0 && invId) {
                ...
                let vendPayID = ETRIS_MODULE.createBillPayment(vendBillId, _date, apAccount, bankAccount, jsondata, departmentId, 3);
                vbp_id = vendPayID;
                etrismapRecId = mappingRecId;
                _status = "SUCCESS";
        ...
        if (JournalProcess) {
          ...
          if (debit_account != '' && credit_account != '') {
            let jeID = ETRIS_MODULE.createJournalEntry(_trandate, credit_account, debit_account, amount, jsonData, departmentId, 3);
            je_id = jeID;
            etrismapRecId = mappingRecId;
            _status = "SUCCESS";
        ...
        if (CredNoteProcess) {
          if (customerSearchObj.length > 0) {
            ...
            if (invItemSearchObj.length > 0) {
              ...
              let credNoteID = ETRIS_MODULE.createCreditNote(customerId, _trandate, arAccount, itemExternalId, amount, jsonData, departmentId, 3);
              cn_id = credNoteID;
              etrismapRecId = mappingRecId;
              _status = "SUCCESS";
        ...
        if (vendCredProcess) {
          if (vendorSearchObj.length > 0) {
            ...
            if (procItemSearchObj.length > 0) {
              ...
              let venCredId = ETRIS_MODULE.createVendorCredit(vendorId, _trandate, apAccount, procItemId, amount, jsonData, departmentId, 3, isSST);
              vc_id = venCredId;
              etrismapRecId = mappingRecId;
              _status = "SUCCESS";
        ...
        if (DebNoteProcess) {
          if (customerSearchObj.length > 0) {
            ...
            if (invItemSearchObj.length > 0) {
              ...
              let dnId = ETRIS_MODULE.createDebitNote(customerId, _trandate, arAccount, _invItemId, amount, jsonData, departmentId, 3);
              dn_id = dnId;
              etrismapRecId = mappingRecId;
              _status = "SUCCESS";
