/*******************************************************************************
 * @Client        :  HRDC
 * @Script Name   :  etris_rl_send_bank_adjustment.js
 * @script Record :  - ETRIS RL | Send Bank Adjustment
 * @Trigger Type  :  post
  ...
  function _post(context) {
    try {
      ...
      let folderId = scriptObj.getParameter({
        name: 'custscript_etris_rl_folder_id4'
      });
      ...
      let fileId = file.create({
        name: "Bank_Adjustment_" + _date.getDate() + '_' + (_date.getMonth() + 1) + '_' + _date.getFullYear() + '_' + _date.getTime(),
      ...
      etRisJsonRecObj.setValue('custrecord_sol_etris_process', 5); //5. Bank Adjustment
      ...
      etRisJsonRecObj.setValue('custrecord_sol_etris_process_name', context.bank_adjustment_service.bank_adjustment.process_name);
      etRisJsonRecObj.setValue('custrecord_sol_etris_transaction_scheme', context.bank_adjustment_service.bank_adjustment.transaction_scheme);
      etRisJsonRecObj.setValue('custrecord_sol_etris_isrefund', context.bank_adjustment_service.bank_adjustment.refund);
      etRisJsonRecObj.setValue('custrecord_sol_etris_cancellation', context.bank_adjustment_service.bank_adjustment.cancellation);
      var statusRecId = etRisJsonRecObj.save();

      var jsonData = {
        'custbody_sol_etris_reference_number': context.bank_adjustment_service.bank_adjustment.reference_number,
        'custbody_sol_etris_process_name': context.bank_adjustment_service.bank_adjustment.process_name,
        'custbody_sol_etris_transaction_remark_': context.bank_adjustment_service.bank_adjustment.transaction_remarks,
        'custbody_sol_etris_transaction_scheme': context.bank_adjustment_service.bank_adjustment.transaction_scheme,
        'custbody_sol_etris_refund': context.bank_adjustment_service.bank_adjustment.refund,
        'custbody_sol_etris_cancellation_': context.bank_adjustment_service.bank_adjustment.cancellation,
        'custbody_sol_etris_my_co_id': context.bank_adjustment_service.bank_adjustment.my_co_id,
        'custbody_sol_etris_cust_supplier_name_': context.bank_adjustment_service.bank_adjustment.customer_or_supplier_name,
        'custbody_sol_etris_payment_amount': context.bank_adjustment_service.bank_adjustment.payment_amount,
        'custbody_sol_etris_payment_date': context.bank_adjustment_service.bank_adjustment.payment_date,
        'custbody_sol_etris_employer_state': context.bank_adjustment_service.bank_adjustment.employer_state
      };
      log.debug("jsonData", JSON.stringify(jsonData));

      let eTrisMapperSearchObj = searchEtrisMapper(5, jsonData.custbody_sol_etris_process_name, jsonData.custbody_sol_etris_cancellation_, jsonData.custbody_sol_etris_refund, jsonData.custbody_sol_etris_transaction_scheme); //processName, isCancel, isrefund, scheme
      ...
      if (eTrisMapperSearchObj.length > 0) {
        ...
        var arAccount = 2922;
        var apAccount = 2821;
        var _trandate = (jsonData.custbody_sol_etris_payment_date).replaceAll('-', '/');
        log.debug('trandate', _trandate);
        var amount = jsonData.custbody_sol_etris_payment_amount;
        ...
        var customerSearchObj = search.create({
          ...
          filters: [
            ['custentity_sol_ertis_my_co_id', 'is', jsonData.custbody_sol_etris_my_co_id]
          ],
        ...
        if (invProcess) {
          if (customerSearchObj.length > 0) {
            ...
            if (_sourceArray[1] == "Scheme_Source") {
              ...
              let _scheme = eTrisMapperSearchObj[0].getValue('custrecord_sol_etris_scheme') ? eTrisMapperSearchObj[0].getValue('custrecord_sol_etris_scheme') : jsonData.custbody_sol_etris_transaction_scheme;
            ...
            if (invItemSearchObj.length > 0) {
              ...
              let invId = ETRIS_MODULE.createInvoice(customerId, _trandate, arAccount, _invItemId, amount, jsonData, departmentId, 5, isSST);
              inv_id = invId;
        ...
        if (custPaymentProcess) {
          if (customerSearchObj.length > 0) {
            ...
            if (invItemSearchObj.length > 0) {
              ...
              let invId = ETRIS_MODULE.createInvoice(customerId, _trandate, arAccount, _invItemId, amount, jsonData, departmentId, 5);
              inv_id = invId;
              ...
              var cpSchemeSearchObj = search.create({
                type: 'customrecord_sol_etris_schemesource',
                filters: [
                  ['name', 'is', jsonData.custbody_sol_etris_transaction_scheme]
                ],
              ...
              if (cpSchemeSearchObj.length > 0 && invId) {
                ...
                let custPayID = ETRIS_MODULE.createCustomerPayment(invId, _trandate, arAccount, bankAccount, jsonData, departmentId, 5);
                cpay_id = custPayID;
                etrismapRecId = mappingRecId;
                _status = "SUCCESS";
        ...
        if (vendBillProcess) {
          if (vendorSearchObj.length > 0) {
            ...
            if (procItemSearchObj.length > 0) {
              ...
              let vendBillId = ETRIS_MODULE.createVendorBill(vendorId, _trandate, apAccount, procItemId, amount, jsonData, departmentId, 5);
              vb_id = vendBillId;
              etrismapRecId = mappingRecId;
              _status = "SUCCESS";
          ...
          if (vendorSearchObj.length > 0) {
            ...
            if (procItemSearchObj.length > 0) {
              ...
              if (schemeSearchObj.length > 0 && invId) {
                ...
                let vendPayID = ETRIS_MODULE.createBillPayment(vendBillId, _date, apAccount, bankAccount, jsondata);
                vbp_id = vendPayID;
                etrismapRecId = mappingRecId;
                _status = "SUCCESS";
        ...
        if (JournalProcess) {
          ...
          if (debit_account != '' && credit_account != '') {
            let jeID = ETRIS_MODULE.createJournalEntry(_trandate, credit_account, debit_account, amount, jsonData, departmentId, 5);
            je_id = jeID;
            etrismapRecId = mappingRecId;
            _status = "SUCCESS";
        ...
        if (CredNoteProcess) {
          if (customerSearchObj.length > 0) {
            ...
            if (invItemSearchObj.length > 0) {
              ...
              let credNoteID = ETRIS_MODULE.createCreditNote(customerId, _trandate, arAccount, itemExternalId, amount, jsonData, departmentId, 5);
              cn_id = credNoteID;
              etrismapRecId = mappingRecId;
              _status = "SUCCESS";
        ...
        if (vendCredProcess) {
          if (vendorSearchObj.length > 0) {
            ...
            if (procItemSearchObj.length > 0) {
              ...
              let venCredId = ETRIS_MODULE.createVendorCredit(vendorId, _trandate, apAccount, procItemId, amount, jsonData, departmentId, 5, isSST);
              vc_id = venCredId;
              etrismapRecId = mappingRecId;
              _status = "SUCCESS";
        ...
        if (DebNoteProcess) {
          if (customerSearchObj.length > 0) {
            ...
            if (invItemSearchObj.length > 0) {
              ...
              let dnId = ETRIS_MODULE.createDebitNote(customerId, _trandate, arAccount, _invItemId, amount, jsonData, departmentId, 5);
              dn_id = dnId;
              etrismapRecId = mappingRecId;
              _status = "SUCCESS";
