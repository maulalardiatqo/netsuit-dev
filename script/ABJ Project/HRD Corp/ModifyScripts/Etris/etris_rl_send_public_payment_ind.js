/*******************************************************************************
 * @Client        :  HRDC
 * @Script Name   :  etris_rl_send_public_payment_ind.js
 * @script Record :  - ETRIS RL | Send Public Payment Ind
 * @Trigger Type  :  post
  ...
  function _post(context) {
    try {
      ...
      let folderId = scriptObj.getParameter({
        name: 'custscript_etris_rl_folder_id5'
      });
      ...
      let fileId = file.create({
        name: "Public_Payment_Ind_" + _date.getDate() + '_' + (_date.getMonth() + 1) + '_' + _date.getFullYear() + '_' + _date.getTime(),
      ...
      etRisJsonRecObj.setValue('custrecord_sol_etris_process', 6); //6. Public Payment Ind
      ...
      etRisJsonRecObj.setValue('custrecord_sol_etris_process_name', context.payment_by_inv.payment.process_name);
      etRisJsonRecObj.setValue('custrecord_sol_etris_transaction_scheme', context.payment_by_inv.payment.scheme);
      etRisJsonRecObj.setValue('custrecord_sol_etris_isrefund', context.payment_by_inv.payment.refund);
      etRisJsonRecObj.setValue('custrecord_sol_etris_cancellation', context.payment_by_inv.payment.cancellation);
      var statusRecId = etRisJsonRecObj.save();
      var isSST = context.payment_by_inv.payment['sst_registered'] == "YES";

      var jsonData = {
        'custbody_sol_etris_reference_number': context.payment_by_inv.payment.reference_number,
        'custbody_sol_etris_customer_type': context.payment_by_inv.payment.customer_type,
        'custbody_sol_etris_my_kad_num': context.payment_by_inv.payment.my_kad_num,
        'custbody_sol_etris_passport_num': context.payment_by_inv.payment.passport_num,
        'custbody_sol_etris_participant_email': context.payment_by_inv.payment.participant_email,
        'custbody_sol_etris_scheme': context.payment_by_inv.payment.scheme,
        'custbody_sol_etris_approved_amount_': context.payment_by_inv.payment.approved_amount,
        'custbody_sol_etris_approved_date': context.payment_by_inv.payment.approved_date,
        'custbody_sol_etris_process_name': context.payment_by_inv.payment.process_name,
        'custbody_sol_etris_customer_id': context.payment_by_inv.payment.customer_id,
        'custbody_sol_etris_typeof_payment_': context.payment_by_inv.payment.typeof_payment,
        'custbody_sol_etris_bank_code': context.payment_by_inv.payment.bank_code,
        'custbody_sol_etris_bank_branch': context.payment_by_inv.payment.bank_branch,
        'custbody_sol_etris_refund': context.payment_by_inv.payment.refund,
        'custbody_sol_etris_cancellation_': context.payment_by_inv.payment.cancellation,
        'custbody_sol_etris_event_id': context.payment_by_inv.payment.event_id,
        'custbody_sol_etris_event_reg_id': context.payment_by_inv.payment.event_registration_id
      };
      log.debug("jsonData", JSON.stringify(jsonData));

      let eTrisMapperSearchObj = searchEtrisMapper(6, jsonData.custbody_sol_etris_process_name, jsonData.custbody_sol_etris_cancellation_, jsonData.custbody_sol_etris_refund, jsonData.custbody_sol_etris_scheme); //processName, isCancel, isrefund, scheme
      ...
      if (eTrisMapperSearchObj.length > 0) {
        ...
        if (invProcess) {
          if (customerSearchObj.length > 0) {
            ...
            if (invItemSearchObj.length > 0) {
              ...
              let invId = ETRIS_MODULE.createInvoice(customerId, _trandate, arAccount, _invItemId, amount, jsonData, departmentId, 6, isSST);
              inv_id = invId;
        ...
        if (custPaymentProcess) {
          if (customerSearchObj.length > 0) {
            ...
            if (invItemSearchObj.length > 0) {
              ...
              let invId = ETRIS_MODULE.createInvoice(customerId, _trandate, arAccount, _invItemId, amount, jsonData, departmentId, 6);
              inv_id = invId;
              ...
              if (cpSchemeSearchObj.length > 0 && invId) {
                ...
                let custPayID = ETRIS_MODULE.createCustomerPayment(invId, _trandate, arAccount, bankAccount, jsonData, departmentId, 6);
                cpay_id = custPayID;
                etrismapRecId = mappingRecId;
                _status = "SUCCESS";
        ...
        if (vendBillProcess) {
          if (vendorSearchObj.length > 0) {
            ...
            if (procItemSearchObj.length > 0) {
              ...
              let vendBillId = ETRIS_MODULE.createVendorBill(vendorId, _trandate, apAccount, procItemId, amount, jsonData, departmentId, 6);
              vb_id = vendBillId;
              etrismapRecId = mappingRecId;
              _status = "SUCCESS";
          ...
          if (vendorSearchObj.length > 0) {
            ...
            if (procItemSearchObj.length > 0) {
              ...
              if (schemeSearchObj.length > 0 && invId) {
                ...
                let vendPayID = ETRIS_MODULE.createBillPayment(vendBillId, _date, apAccount, bankAccount, jsondata, departmentId, 6);
                vbp_id = vendPayID;
                etrismapRecId = mappingRecId;
                _status = "SUCCESS";
        ...
        if (JournalProcess) {
          ...
          if (debit_account != '' && credit_account != '') {
            let jeID = ETRIS_MODULE.createJournalEntry(_trandate, credit_account, debit_account, amount, jsonData, departmentId, 6);
            je_id = jeID;
            etrismapRecId = mappingRecId;
            _status = "SUCCESS";
        ...
        if (CredNoteProcess) {
          if (customerSearchObj.length > 0) {
            ...
            if (invItemSearchObj.length > 0) {
              ...
              let credNoteID = ETRIS_MODULE.createCreditNote(customerId, _trandate, arAccount, itemExternalId, amount, jsonData, departmentId, 6);
              cn_id = credNoteID;
              etrismapRecId = mappingRecId;
              _status = "SUCCESS";
        ...
        if (vendCredProcess) {
          if (vendorSearchObj.length > 0) {
            ...
            if (procItemSearchObj.length > 0) {
              ...
              let venCredId = ETRIS_MODULE.createVendorCredit(vendorId, _trandate, apAccount, procItemId, amount, jsonData, departmentId, 6, isSST);
              vc_id = venCredId;
              etrismapRecId = mappingRecId;
              _status = "SUCCESS";
        ...
        if (DebNoteProcess) {
          if (customerSearchObj.length > 0) {
            ...
            if (invItemSearchObj.length > 0) {
              ...
              let dnId = ETRIS_MODULE.createDebitNote(customerId, _trandate, arAccount, _invItemId, amount, jsonData, departmentId, 6);
              dn_id = dnId;
              etrismapRecId = mappingRecId;
              _status = "SUCCESS";
