/*******************************************************************************
 * @Client        :  HRDC
 * @Script Name   :  etris_rl_send_direct_receipt.js
 * @script Record :  - ETRIS RL | Send Direct Receipt
 * @Trigger Type  :  post
  ...
  function _post(context) {
    try {
      ...
      let folderId = scriptObj.getParameter({
        name: 'custscript_etris_rl_folder_id3'
      });
      ...
      var invId, custPayID, vendBillId, billPayId, jeID, credNoteID, venCredId, debNoteId;
      ...
      let fileId = file.create({
        name: "Direct_Receipt_" + _date.getDate() + '_' + (_date.getMonth() + 1) + '_' + _date.getFullYear() + '_' + _date.getTime(),
      ...
      etRisJsonRecObj.setValue('custrecord_sol_etris_process', 4); //4. Direct Receipt
      ...
      etRisJsonRecObj.setValue('custrecord_sol_etris_process_name', context.direct_receipt.receipt.process_name);
      etRisJsonRecObj.setValue('custrecord_sol_etris_transaction_scheme', context.direct_receipt.receipt.scheme);
      etRisJsonRecObj.setValue('custrecord_sol_etris_isrefund', context.direct_receipt.receipt.refund);
      etRisJsonRecObj.setValue('custrecord_sol_etris_cancellation', context.direct_receipt.receipt.cancellation);
      var statusRecId = etRisJsonRecObj.save();

      var jsonData = {
        'custbody_sol_etris_customer_type': context.direct_receipt.receipt.customer_type,
        'custbody_sol_etris_reference_number': context.direct_receipt.receipt.reference_number,
        'custbody_sol_etris_participant_email': context.direct_receipt.receipt.participant_email,
        'custbody_sol_etris_scheme': context.direct_receipt.receipt.scheme,
        'custbody_sol_etris_approved_amount_': context.direct_receipt.receipt.approved_amount,
        'custbody_sol_etris_approved_date': context.direct_receipt.receipt.approved_date,
        'custbody_sol_etris_process_name': context.direct_receipt.receipt.process_name,
        'custbody_sol_etris_bank_code': context.direct_receipt.receipt.bank_code,
        'custbody_sol_etris_bank_branch': context.direct_receipt.receipt.bank_branch,
        'custbody_sol_etris_cheque_number': context.direct_receipt.receipt.cheque_number,
        'custbody_sol_etris_my_co_id': context.direct_receipt.receipt.my_co_id,
        'custbody_sol_etris_refund': context.direct_receipt.receipt.refund,
        'custbody_sol_etris_cancellation_': context.direct_receipt.receipt.cancellation,
        'custbody_sol_etris_cheque': context.direct_receipt.receipt.type_of_payment
      };
      log.debug("jsonData", JSON.stringify(jsonData));

      let eTrisMapperSearchObj = searchEtrisMapper(4, jsonData.custbody_sol_etris_process_name, jsonData.custbody_sol_etris_cancellation_, jsonData.custbody_sol_etris_refund, jsonData.custbody_sol_etris_scheme); //processName, isCancel, isrefund, scheme
      ...
      if (eTrisMapperSearchObj.length > 0) {
        ...
        if (invProcess) {
          if (customerSearchObj.length > 0) {
            ...
            if (invItemSearchObj.length > 0) {
              ...
              let invId = ETRIS_MODULE.createInvoice(customerId, _trandate, arAccount, _invItemId, amount, jsonData, departmentId, 4, isSST);
              inv_id = invId;
        ...
        if (custPaymentProcess) {
          if (customerSearchObj.length > 0) {
            ...
            if (invItemSearchObj.length > 0) {
              ...
              let invId = ETRIS_MODULE.createInvoice(customerId, _trandate, arAccount, _invItemId, amount, jsonData, departmentId, 4);
              inv_id = invId;
              ...
              if (cpSchemeSearchObj.length > 0 && invId) {
                ...
                let custPayID = ETRIS_MODULE.createCustomerPayment(invId, _trandate, arAccount, bankAccount, jsonData, departmentId, 4);
                cpay_id = custPayID;
                etrismapRecId = mappingRecId;
                _status = "SUCCESS";
        ...
        if (vendBillProcess) {
          if (vendorSearchObj.length > 0) {
            ...
            if (procItemSearchObj.length > 0) {
              ...
              let vendBillId = ETRIS_MODULE.createVendorBill(vendorId, _trandate, apAccount, procItemId, amount, jsonData, departmentId, 4);
              vb_id = vendBillId;
              etrismapRecId = mappingRecId;
              _status = "SUCCESS";
          ...
          if (vendorSearchObj.length > 0) {
            ...
            if (procItemSearchObj.length > 0) {
              ...
              if (schemeSearchObj.length > 0 && invId) {
                ...
                let vendPayID = ETRIS_MODULE.createBillPayment(vendBillId, _date, apAccount, bankAccount, jsondata, departmentId, 4);
                vbp_id = vendPayID;
                etrismapRecId = mappingRecId;
                _status = "SUCCESS";
        ...
        if (JournalProcess) {
          ...
          if (debit_account != '' && credit_account != '') {
            let jeID = ETRIS_MODULE.createJournalEntry(_trandate, credit_account, debit_account, amount, jsonData, departmentId, 4);
            je_id = jeID;
            etrismapRecId = mappingRecId;
            _status = "SUCCESS";
        ...
        if (CredNoteProcess) {
          if (customerSearchObj.length > 0) {
            ...
            if (invItemSearchObj.length > 0) {
              ...
              let credNoteID = ETRIS_MODULE.createCreditNote(customerId, _trandate, arAccount, itemExternalId, amount, jsonData, departmentId, 4);
              cn_id = credNoteID;
              etrismapRecId = mappingRecId;
              _status = "SUCCESS";
        ...
        if (vendCredProcess) {
          if (vendorSearchObj.length > 0) {
            ...
            if (procItemSearchObj.length > 0) {
              ...
              let venCredId = ETRIS_MODULE.createVendorCredit(vendorId, _trandate, apAccount, procItemId, amount, jsonData, departmentId, 4, isSST);
              vc_id = venCredId;
              etrismapRecId = mappingRecId;
              _status = "SUCCESS";
        ...
        if (DebNoteProcess) {
          if (customerSearchObj.length > 0) {
            ...
            if (invItemSearchObj.length > 0) {
              ...
              let dnId = ETRIS_MODULE.createDebitNote(customerId, _trandate, arAccount, _invItemId, amount, jsonData, departmentId, 4);
              dn_id = dnId;
              etrismapRecId = mappingRecId;
              _status = "SUCCESS";
