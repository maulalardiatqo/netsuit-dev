...
define(["N/search", "N/record", "N/ui/serverWidget"], function(search, record, serverWidget) {
  ...
  function onRequest(context) {
    try {
      ...
      } else if ((context.request.method == "GET" && customer_param) || context.request.method == "POST") {
        //update
  ...
  function createSublist(sublistname, form, filterPayMethodData, defaultPaymentMethod) {
    ...
    var reference = sublist.addField({
      id: "custpage_reference",
      type: serverWidget.FieldType.TEXT,
      label: "Reference#",
    });
    reference.updateDisplayType({
      displayType: serverWidget.FieldDisplayType.ENTRY
    });

    var costcenter = sublist.addField({
      id: "custpage_costcenter",
      type: serverWidget.FieldType.SELECT,
      label: "Cost Center",
      source: "customrecord_cseg_abj_cc"
    });
    costcenter.updateDisplayType({
      displayType: serverWidget.FieldDisplayType.ENTRY
    });
