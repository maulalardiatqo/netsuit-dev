...
define(["N/search", "N/ui/message"], function(search, message) {
  ...
  function markall() {
    ...
    var count = nlapiGetLineItemCount('custpage_pay_list'); //gets the count of lines
    for (var i = 1; i <= count; i++) {
      nlapiSelectLineItem('custpage_pay_list', i);
      nlapiSetCurrentLineItemValue('custpage_pay_list', 'custpage_checkbox', 'T', true, true); //'custcol_checkbox_field' is checkbox's field ID.
      console.log("markall");
    }
    nlapiCommitLineItem('custpage_pay_list');
  ...
  function unmarkall() {
    ...
    for (var i = 1; i <= count; i++) {
      nlapiSelectLineItem('custpage_pay_list', i);
      nlapiSetCurrentLineItemValue('custpage_pay_list', 'custpage_checkbox', 'F', false, false); //'custcol_checkbox_field' is checkbox's field ID.
      nlapiSetCurrentLineItemValue('custpage_pay_list', 'custpage_pay_amt', '');
  ...
  function createPayment() {
    ...
    if (totalNok <= 0) {
      ...
      alert(`Please select payment method for Invoice NO ${bodyAlert}`);
    }
  }
