 ...
 */
      // N/record = data N/search = query N
    ...
    try {
      ...
      if (olddelegateTo || delegateTo) {
        ...
        if ((delegateTo !== olddelegateTo) && olddelegateTo) {
          ...
          var getDelegated = empRecordtoclear.getValue({fieldId:'custentity_emp_delegation'});
          log.debug('delegasi', getDelegated);
          var delegateApproval = empRecord.getValue({fieldId:'custentity_emp_delegation'});
          var empty = [];

          for (let index = 0; index < getDelegated.length; index++) {
            const element = getDelegated[index];
            for (let indo = 0; indo < delegateApproval.length; indo++) {
              const el = delegateApproval[indo];
              if(element !== el){
                empty.push(element)
              }
            }
		  ...
		  log.debug('emprec_id to clear', emprec_id);
        }
        ...
        var list_empApproval_id = empRecord.getValue("custentity_emp_delegation");//[];
		    log.debug("list_empApproval_id 1", list_empApproval_id);
          ...
          EmployeeApprovals.forEach(function(EmployeeApproval) {
            ...
            if (IsDelegated) {
              ...
              log.debug("undelegate", true);
			      let idxToDelete = list_empApproval_id.indexOf(GrpApprovlId);
			      list_empApproval_id.splice(idxToDelete, 1);
            log.debug('cobacek', list_empApproval_id);
            ...
            empRecordtoclear.setValue({
              fieldId: 'custentity_emp_delegation',
              value: list_empApproval_id
            });

            empRecordtoclear.save({
              enableSourcing: true,
              ignoreMandatoryFields: true
            })
