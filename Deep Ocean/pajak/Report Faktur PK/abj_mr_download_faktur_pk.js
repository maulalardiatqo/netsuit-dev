...
define(['N/search', 'N/runtime', 'N/file', 'N/log', 'N/format'], function (search, runtime, file, log, format) {
    ...
    function getInputData() {
        ...
        const fakturSearch = search.load({ id: 'customsearch_bs_faktur' });
        const fakturDetailSearch = search.load({ id: 'customsearch_bs_faktur_detail' });
    ...
    function reduce(context) {
        ...
        if (jobAction === 'excel') {
                ...
                getText(grouped.faktur.values.custbody_bs_jenis_fp),
                grouped.faktur.values.custbody_bs_kode_transaksi_trx || '',
                getText(grouped.faktur.values.custbody_bs_ket_tamb),
                grouped.faktur.values.custbody_bs_dok_pendukung || '',
                grouped.faktur.values.custbody_bs_period_dok_pendukung || '',
                grouped.faktur.values.invoicenum || '',
                getText(grouped.faktur.values.custbody_bs_cap_fasilitas) || '',
                grouped.faktur.values.custbody_bs_id_tku_sales || '',
                ...
                grouped.faktur.values.custbody_bs_id_tku_pembeli_trx || '',
            ] : null;
        ...
        if (jobAction === 'xml' && grouped.faktur) {
            ...
            xmlParts.push(`<TaxInvoiceOpt>${getText(header.custbody_bs_jenis_fp)}</TaxInvoiceOpt>`);
            xmlParts.push(`<TrxCode>${header.custbody_bs_kode_transaksi_trx}</TrxCode>`);
            xmlParts.push(`<AddInfo>${getText(header.custbody_bs_ket_tamb)}</AddInfo>`);
            xmlParts.push(`<CustomDoc>${header.custbody_bs_dok_pendukung || ''}</CustomDoc>`);
            xmlParts.push(`<CustomDocMonthYear>${header.custbody_bs_period_dok_pendukung || ''}</CustomDocMonthYear>`);
            xmlParts.push(`<RefDesc>${header.invoicenum|| ''}</RefDesc>`);
            xmlParts.push(`<FacilityStamp>${getText(header.custbody_bs_cap_fasilitas) || ''}</FacilityStamp>`);
            xmlParts.push(`<SellerIDTKU>${header.custbody_bs_id_tku_sales || ''}</SellerIDTKU>`);
            ...
            xmlParts.push(`<BuyerIDTKU>${header.custbody_bs_id_tku_pembeli_trx || ''}</BuyerIDTKU>`);
            ...
            grouped.details.forEach(detail => {
                ...
                xmlParts.push(`<Opt>${getText(v.custbody_bs_barang_jasa)}</Opt>`);
