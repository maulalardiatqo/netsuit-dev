    ...
    function(render, search, record, log, file, http, config, format, email, runtime) {
        ...
        function onRequest(context) {
            ...
            if(subsidiari){
                ...
                log.debug('subsidiariRec', subsidiariRec)
                // load for header
            ...
            // load vendor
            var vendor_id = vencredRecord.getValue('entity');
            ...
            // PO data
            var tandId = vencredRecord.getValue('transactionnumber');
            var POdate = vencredRecord.getValue('trandate');
            var termsCek = vencredRecord.getText('terms');
            if(termsCek){
                terms = termsCek
            }else{
                terms = ''
            }
            var poTotal = vencredRecord.getValue('total') || 0;
            var taxtotal = vencredRecord.getValue('taxtotal') || 0;
            ...
            var countItem = vencredRecord.getLineCount({
                sublistId: 'item'
            });
            if(countItem > 0){
                var taxpphList = [];
                for (var i = 0; i < countItem; i++) {
                    var taxpph = vencredRecord.getSublistValue({
                        sublistId: 'item',
                        fieldId: 'custcol_4601_witaxrate',
                        line: i
                    });
                    whtaxammountItem = vencredRecord.getSublistValue({
                        sublistId: 'item',
                    ...
                    var taxRate = vencredRecord.getSublistValue({
                        sublistId: 'item',
                        fieldId: 'taxrate1',
                        line: i
                    });
            ...
            var countExpense = vencredRecord.getLineCount({
                sublistId: 'expense'
            });
            ...
            if(countExpense > 0){
                ...
                for (var i = 0; i < countExpense; i++) {
                    ...
                    whtaxammountExp = vencredRecord.getSublistValue({
                        sublistId: 'expense',
                        fieldId: 'custcol_4601_witaxamt_exp',
                        line: i
                    });
                    var whTaxCode = vencredRecord.getSublistValue({
                        sublistId: 'expense',
            ...
            log.debug('taxpphToPrint out', taxpphToPrint)
            ...
            log.debug('totalToCount', totalToCount);
            log.debug('tamount', tamount);
            ...
            body += "</td>"
            body += "<td>"
            body += "<p class='tg-headerrow_legalName'> Bill Credit # : "+ tandId + "<br/>"
            body += ""+ POdate + "</p>"
            ...
            body += "</tr>"
            body += getPOItem(context, vencredRecord);
            body += getPOExpense(context, vencredRecord);
            body += "<tr>"
            ...
            body += "<td style='width:82%'></td>"
            body += "</tr>"
            body += "<tr>"
            body += "<td></td>"
            body += "<td class='tg-headerrow_left'>Reference :</td>"
            if(refno){
                body += "<td class='tg-headerrow_left'> "+refno+"</td>"
            }
            body += "</tr>"
            body += "</tbody>";
            body += "</table>";
        ...
        function getPOItem(context, vencredRecord){
            var itemCount = vencredRecord.getLineCount({
                sublistId: 'item'
            });
            ...
            if(itemCount > 0){
                ...
                for(var index = 0; index < itemCount; index++){
                    var qty = vencredRecord.getSublistValue({
                        sublistId: 'item',
                        fieldId: 'quantity',
                        line: index
                    });
                    ...
                    var rate = vencredRecord.getSublistValue({
                        sublistId: 'item',
                        fieldId: 'rate',
                        line: index
                    });
                    ...
                    var taxAmt = vencredRecord.getSublistValue({
                        sublistId: 'item',
                        fieldId: 'tax1amt',
                        line: index
                    });
                    ...
                    var ammount = vencredRecord.getSublistValue({
                        sublistId: 'item',
                        fieldId: 'grossamt',
                        line: index
                    });
        ...
        function getPOExpense(context, vencredRecord){
            var expCont = vencredRecord.getLineCount({
                sublistId : 'expense'
            });
            if(expCont > 0){
                var body = "";
                for(var index = 0; index < expCont; index++){
                    ...
                    var taxamt_exp = vencredRecord.getSublistValue({
                        sublistId: 'expense',
                        fieldId: 'tax1amt',
                        line: index
                    });
                    ...
                    var grosamt_exp = vencredRecord.getSublistValue({
                        sublistId: 'expense',
                        fieldId: 'grossamt',
                        line: index
                    });
