    ...
    function (render, search, record, log, file, http, config, format, email, runtime) {
        try{
            ...
            function onRequest(context) {
                ...
                // load PO
                var poSearchHead = search.load({
                    id : 'customsearch1536'
                });
                if(recid){
                    poSearchHead.filters.push(
                        search.createFilter({
                        name: "internalid",
                ...
                var poSearchHeadet = poSearchHead.run();
                var poRecord = poSearchHeadet.getRange(0, 1);
                ...
                if (vendor_id) {
                    ...
                    if (isperson == 'T') {
                        ...
                        log.debug('isChecklist', isChecklist)
    
                        if (isChecklist === true) {
                            log.debug('masuk kondisi true')
                ...
                // 1. Load Saved Search yang sudah ada
                var transactionSearchObj = search.load({
                    id: 'customsearch_po_print_line'
                });
                ...
                transactionSearchObj.run().each(function(result) {
                    ...
                    var whCodeName = result.getValue({
                        name: "custrecord_4601_wtc_name",
                        join: "CUSTCOL_4601_WITAXCODE"
                    });

                    if (whCodeName) {
                        whTaxCodetoPrint = whCodeName;
                    ...
                    if (taxRateItem != 0 && taxRateList.indexOf(taxRateItem) === -1) {
                        taxRateList.push(parseFloat(taxRateItem));
                    }
